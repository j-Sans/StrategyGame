...
//Functions
void updateMouse();
void updateBoard(Host* host);
std::vector<std::vector<Tile> > getBoard();
void setAddplayerThread(Host* host);

void host(Host* hostPointer, bool* hasBegun);
                ...
                while (!M.getShouldWindowClose()) {
                    ...
                    if (action == READY_TO_PLAY) {
                        ...
                        if (runningHost == false) {
                            H.set(3000);
                            updateBoard(&H);
//                            hostThread = std::thread(host, hostPointer);
                            runningHost = true;
                        }
//                        if (hostPointer != nullptr) {
                            M.numberOfConnections = H.getNumberPlayers();
//                            M.numberOfConnections = hostPointer->getNumberPlayers();
//                        }
                    } else if (action == ADD_PLAYER) {
                        H.addPlayer();
//                        hostPointer->addPlayer();
                    }
                }
...
//                    break;
                ...
                std::thread thread(setAddplayerThread, &H);
...
void updateBoard(Host* host) {
    ...
    host->board.setCreature(Creature(0, BOARD_WIDTH - 2, Human, 3, 3, 2, Melee, 1, 30, NORTH, 0));
    host->board.setCreature(Creature(1, BOARD_WIDTH - 2, Human, 3, 3, 2, Melee, 1, 1, NORTH, 0));
    host->board.setCreature(Creature(1, BOARD_WIDTH - 1, Human, 3, 3, 2, Melee, 1, 1, NORTH, 0));
    
    host->board.setCreature(Creature(BOARD_WIDTH - 1, 1, Human, 3, 3, 2, Melee, 1, 1, NORTH, 1));
    host->board.setCreature(Creature(BOARD_WIDTH - 2, 1, Human, 3, 3, 2, Melee, 1, 1, NORTH, 1));
    host->board.setCreature(Creature(BOARD_WIDTH - 2, 0, Human, 3, 3, 2, Melee, 1, 1, NORTH, 1));
    ...
    host->board.setBuilding(player0Home);
    host->board.setBuilding(player1Home);
}
